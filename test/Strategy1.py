...
def BoxTheory(df,N,S):
    ...
    for i in  range( len(df)):
        ...
        if (df['BoxTop'].iloc[i] < df['Close'].iloc[i] ) &  (df['BoxTopDef'].iloc[i] > S) & (df['ma_s'].iloc[i] < df['Close'].iloc[i]) & (df['rsi'].iloc[i] < 90  and df['rsi'].iloc[i] > 10) :
            df['box_sign'].iloc[i] = 1
           
    
        
        #設定Box指標箱型突破低點訊號= -1 (部位買進)
        if (df['BoxDown'].iloc[i] > df['Close'].iloc[i])  &  (df['BoxDownDef'].iloc[i] > S ) & (df['ma_s'].iloc[i] > df['Close'].iloc[i]) & (df['rsi'].iloc[i] < 90  and df['rsi'].iloc[i] > 10) : 
            df['box_sign'].iloc[i] = -1
