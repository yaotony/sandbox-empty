...
for i in range (len(filenames)):
    ...
    data = df[ (df['商品代號']==Broker) & (df['到期月份(週別)']==YYMM) & (df['成交日期']==yy+mm+dd)   ]
    ...
    for i in  range( len(data)):
        ...
        if close1 >0 and close2 >0  :
            ...
            if i < L-1 :
                ...
                if b == 1  or b == -1 :
                    if b == 1 :
                        (r,b,sr,sb,topProfit,rr,srr,note)=stopByB(Price,1,-0.25,r,b,sr,sb,topProfit,note,Time)
                    elif b==-1 :
                        (r,b,sr,sb,topProfit,rr,srr,note)=stopByS(Price,1,-0.25,r,b,sr,sb,topProfit,note,Time)
